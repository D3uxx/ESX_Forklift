...
function refreshBlips()
	if playerData.job.name ~= nil and playerData.job.name == 'fork' then 	
		drawBlip(Config.locker, 280, "Trukkikuskin pukukoppi")			 	
		drawBlip(Config.carSpawner, 479, "Trukin lunastus")					
		drawBlip(Config.carDelete, 490, "Trukin palautus")				
	end
end
...
function getPaid()
	...
	if IsPedInAnyVehicle(GetPlayerPed(-1)) and isMyCar() then			
		...
		ESX.ShowNotification('~w~Voittosumma: ~g~ +' .. pay .. ' ~w~euroa.')
	else														
...
Citizen.CreateThread(function()
	while true do																
		...
		if not menuIsOpen then
			local playerCoords = GetEntityCoords(GetPlayerPed(-1))
			if playerData.job ~= nil and playerData.job.name == "fork" and playerIsInside(playerCoords, Config.locker, 2.5) then 				
				isInMarker = true
				displayHint = true																
			...
			elseif onDuty and currentJob == 'deliver' and taskPoints['deliver'] ~= nil and playerIsInside(playerCoords, taskPoints['deliver'], Config.pickupDistance) then
				...
				currentZone = 'none' 
			elseif playerData.job ~= nil and playerData.job.name == "fork" and currentPlate ~= '' and playerIsInside(playerCoords, Config.carDelete, 1.5) then  				
				isInMarker = true
				displayHint = true
...
Citizen.CreateThread(function()
	while true do																			
		Citizen.Wait(1)
		local playerCoords = GetEntityCoords(GetPlayerPed(-1))
		if playerData.job ~= nil and playerData.job.name == "fork" and playerIsInside(playerCoords, Config.locker, 100) then 
		...
		if playerData.job ~= nil and playerData.job.name == "fork" and onDuty and currentPlate ~= '' and playerIsInside(playerCoords, Config.carDelete, 100) then  		
...
function openMenu()									
  ...
  ESX.UI.Menu.Open(
    'default', GetCurrentResourceName(), 'locker',			
    {
      title    = "Pukukoppi",							
      elements = {
        {label = "Työvaatteet", value = 'fork_wear'},		
